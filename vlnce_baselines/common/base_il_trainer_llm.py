...
class BaseVLNCETrainerLLM(BaseILTrainer):
    ...
    def _eval_llm(
        ...
        if len(config.VIDEO_OPTION) > 0:
            config.defrost()
            # Only enable custom TopDownMapVLNCE when its graphs file exists; otherwise fallback to official TopDownMap
            try:
        ...
        if config.EVAL.SAVE_RESULTS:
            ...
            if os.path.exists(fname):
                print(f"Evaluation exists at: {fname}")
                # 自动覆盖，无需交互确认
        ...
        while envs.num_envs > 0 and len(stats_episodes) < episodes_to_eval:
            ...
            if not stop_flag:
                ...
                # Get available candidate viewpoints for the prompt
                try:
                    candidate_keys = list(radius_dict.keys())
                except Exception:
                    candidate_keys = []

                predictions, thoughts, break_flag = navigator.move_to_next_vp(nav_logger, current_step, instruction, actions, landmarks, history_traj, estimation, observation, observe_dict, candidate_keys)
                ...
                next_vp, thought, error_number = navigator.test_decisions(nav_logger, fused_pred_thought, observation, instruction, error_number, observe_dict, available_keys)
            ...
            try:
                if not stop_flag:
                    ...
                    # Render frame for video (ego + topdown + instruction text)
                    try:
                        ...
                        # Keep evaluation robust; just skip frame on error
                        pass
                    instruction, images_list = self.generate_input(observations[-1])
                    error_number = 0 
                    # finish navigation
                ...
                for i in range(envs.num_envs):
                    ...
                    # Save video if enabled（自定义帧率与最小时长，不影响其他路径）
                    if len(config.VIDEO_OPTION) > 0 and len(rgb_frames[i]) > 0:
                        try:
                            ...
                            if frame_repeat > 1 and len(frames) > 0:
                                ...
                                frames = repeated
                            # 不做末尾补帧，episode 结束视频即结束

                            video_name = f"episode={current_episodes[i].episode_id}-ckpt=0-spl={metric['spl']:.2f}"
                            images_to_video(frames, config.VIDEO_DIR, video_name, fps=desired_fps)
                        except Exception:
                            pass

                        # Also dump trajectory JSON alongside videos under tools/fail/trajectories
                        try:
                            base_dir = config.VIDEO_DIR
                            traj_dir = os.path.join(os.path.dirname(base_dir), 'trajectories') if os.path.basename(base_dir) == 'videos' else os.path.join(base_dir, 'trajectories')
                            os.makedirs(traj_dir, exist_ok=True)
                            traj_payload = {
                                "episode_id": current_episodes[i].episode_id,
                                "positions": positions_.tolist(),
                                "gt_path": gt_path.tolist(),
                                "collisions": collisions_.tolist() if collisions_.size > 0 else [],
                                "metrics": metric,
                            }
                            with open(os.path.join(traj_dir, f"episode_{current_episodes[i].episode_id}.json"), 'w') as tfp:
